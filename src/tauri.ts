...
export type DeepLocalJobSnapshot = {
  job_id: string;
  provider: string;
  state: string;
  message_code: string;
  ...
  result_chars: number;
  usage: DeepLocalUsage;
  warnings: string[];
  state_history: { state: string; at: number }[];
...
export type DeepLocalJobDetail = DeepLocalJobSnapshot & {
  question: string;
...
export type DeepLocalSubmitResult =
  | { ok: true; job: DeepLocalJobSnapshot; duplicate: boolean }
  | { ok: false; error_category: string; error: string };

export async function submitDeepLocalJob(params: {
  question: string;
  evidence?: { source_id: string; snippet: string }[];
  model?: string;
  max_output_tokens?: number;
}): Promise<DeepLocalSubmitResult> {
  return rpc<DeepLocalSubmitResult>("deep_local.submit", params);
}

export async function getDeepLocalJob(jobId: string): Promise<DeepLocalJobDetail> {
  return rpc<DeepLocalJobDetail>("deep_local.get", { job_id: jobId });
}

export async function listDeepLocalJobs(): Promise<DeepLocalJobSnapshot[]> {
  return rpc<DeepLocalJobSnapshot[]>("deep_local.list");
}

export async function cancelDeepLocalJob(jobId: string): Promise<DeepLocalJobSnapshot> {
  return rpc<DeepLocalJobSnapshot>("deep_local.cancel", { job_id: jobId });
}

export async function retryDeepLocalJob(jobId: string): Promise<DeepLocalSubmitResult> {
  return rpc<DeepLocalSubmitResult>("deep_local.retry", { job_id: jobId });
}
