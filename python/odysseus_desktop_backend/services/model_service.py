...
from odysseus_desktop_backend.services.providers.ollama import OllamaProvider
...
class ModelService:
    def __init__(self, db: Database):
        self.db = db
        self._provider = OllamaProvider(OLLAMA_ENDPOINT)
    ...
    def _tcp_reachable(self, host: str, port: int) -> bool:
        return self._provider.tcp_reachable(host, port)

    def _get_json(self, url: str, timeout: float) -> dict[str, Any]:
        return self._provider.get_json(url, timeout)

    def _post_json(self, url: str, payload: dict[str, Any], timeout: float) -> dict[str, Any]:
        return self._provider.post_json(url, payload, timeout)
